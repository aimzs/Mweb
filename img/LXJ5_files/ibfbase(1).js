$(function(){
	...
	function subjectShow(){
		$(".subjec+").animate({"top":"0"},800,function(){
	...
	//二维码
	$(".basehead header .aboutUser .phone .usericon").mouseenter(function(){
	...
	$(".basehead header .aboutUser .user .uesr-box").mousemove(function(){
	...
	function dobbleC(){
		if( flag == 0 ){
			...
			// $("#menuBox").animate({"right":0},800);
			// $("#contentLeft").animate({"left":"-206px"},800);
			flag = 1;
		}else{
			$("#menuBox").css('right', "-2.08rem");
			$("#contentLeft").css('left', 0);
			// $("#menuBox").animate({"right":"-206px"},800);
			// $("#contentLeft").animate({"left":0},800);
			flag = 0;
		}
	}
	...
	//切换方向
	$(".direction .tab-ul .tab-li .link").click(function(){
		var index = $(this).parent().index();
		$(".toggleClassification").eq(index).addClass("toggleClassificationShow").siblings().removeClass("toggleClassificationShow");
...
$(function(){
	var mytop = 1;
	$(".middle-circle  ol li").click(function(){
		mytop++;
		$(this).addClass("current").siblings().removeClass("current");
		var index = $(this).index();
		if( index > mynum){
			  $(".scrollBox").eq(index).css({"left":"101%","zIndex":mytop});
			  $(".h3-box h3").eq(index).css({"left":"101%","zIndex":mytop});	
			  $(".p-box p").eq(index).css({"left":"101%","zIndex":mytop});	
		}else if( index < mynum){
			  $(".scrollBox").eq(index).css({"left":"-101%","zIndex":mytop});
			  $(".h3-box h3").eq(index).css({"left":"-101%","zIndex":mytop});
			  $(".p-box p").eq(index).css({"left":"-101%","zIndex":mytop});	
		}
	    $(".scrollBox").eq(index).animate({"left":"0"},500);
	   	$(".h3-box h3").eq(index).animate({"left":"0"},500);
	   	$(".p-box p").eq(index).animate({"left":"0"},500);
	    num = index;
	});
	var mynum = 0;
	var timer = null;
	function rightPlay(){
		...
		$(".middle-circle  ol li").eq(mynum).addClass("current").siblings().removeClass("current");
		$(".scrollBox").eq(mynum).css({"left":"101%","zIndex":mytop});
		$(".h3-box h3").eq(mynum).css({"left":"101%","zIndex":mytop});
		$(".p-box p").eq(mynum).css({"left":"101%","zIndex":mytop});
	    $(".scrollBox").eq(mynum).animate({"left":"0"},500);
	    $(".h3-box h3").eq(mynum).animate({"left":"0"},500);
	    $(".p-box p").eq(mynum).animate({"left":"0"},500);
	}
	timer = setInterval(rightPlay,4000);
	$("#topPic").mouseover(function(){
		clearInterval(timer);
	});
